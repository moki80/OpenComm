...
/**
 * This abstract class represents a RTP flow for sending or receiving rtp
 * packets.
 * <p>
 * A RtpFlow is always associated to a SSRC number that represents the source
 * end point (the S-SAP).
 */
public abstract class RtpFlow {
	 ...
	 * be followed by exactly one header extension.
	 */
	boolean extension;
	 ...
	 * number SHOULD be random (unpredictable).
	 */
	int sequence_number;
	 ...
	 * synchronization and jitter calculations. If RTP packets are generated
	 * periodically, the nominal sampling instant as determined from the
	 * sampling clock is to be used, not a reading of the system clock. The
	 * initial value of the timestamp SHOULD be random, as for the sequence
	 ...
	 * contributing sources for the payload contained in this packet. CSRC
	 * identifiers are inserted by mixers, using the SSRC identifiers of
	...
	/**
	 * Whether the RTP flow has been initialized, i.e. it has been associated
	 * with a PT, SSRC, sequence number, and timestamp.
	 */
	boolean initialized;
