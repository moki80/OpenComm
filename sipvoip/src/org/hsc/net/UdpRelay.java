...
/**
 * UdpRelay implements a direct UDP datagram relay agent. It receives UDP
 * packets at a local port and relays them toward a remote UDP socket
...
public class UdpRelay extends Thread {
	...
	boolean stop;
	/**
	 * Maximum time that the UDP relay can remain active after been halted (in
	 * milliseconds)
	 */
	int socket_to = 3000; // 3sec
	/**
	 * Maximum time that the UDP relay remains active without receiving UDP
	 * datagrams (in seconds)
	 */
	 ...
	 */
	public UdpRelay(int local_port, String dest_addr, int dest_port,
			UdpRelayListener listener) {
	 ...
	 * Creates a new UDP relay and starts it.
	 * <p>
	 * The UdpRelay will automatically stop after <i>alive_time</i> seconds of
	 * idle time (i.e. without receiving UDP datagrams)
	 */
	public UdpRelay(int local_port, String dest_addr, int dest_port,
			int alive_time, UdpRelayListener listener) {
		init(local_port, dest_addr, dest_port, alive_time, listener);
		start();
	}

	/** Inits a new UDP relay */
	private void init(int local_port, String dest_addr, int dest_port,
			int alive_time, UdpRelayListener listener) {
	...
	/*
	 * public String getDestAddress() { return dest_addr; }
	 */

	/** Gets the destination port */
	/*
	 * public int getDestPort() { return dest_port; }
	 */
	...
	/**
	 * Redirect packets received from remote source addr/port to destination
	 * addr/port
	 */
	public void run() { // System.out.println("DEBUG: starting UdpRelay
						// "+toString()+" (it expires after "+alive_to+" sec)");
		try {
			...
			while (!stop) { // non-blocking receiver
				try {
					socket.receive(packet);
				} catch (InterruptedIOException ie) { // if expired, stop
														// relaying
					if (alive_to > 0 && System.currentTimeMillis() > expire)
						halt();
					continue;
				}
				// check whether the source address and port are changed
				if (src_port != packet.getPort()
						|| !src_addr.equals(packet.getAddress()
								.getHostAddress().toString())) {
					src_port = packet.getPort();
					src_addr = packet.getAddress().getHostAddress();
					if (listener != null)
						listener.onUdpRelaySourceChanged(this, src_addr,
								src_port);
				}
				// relay
	...
	public String toString() {
		return Integer.toString(local_port) + "-->" + dest_addr + ":"
				+ dest_port;
	}
	...
	public static void main(String[] args) {
		...
		int alive_time = 0;
		if (args.length > 3)
			alive_time = Integer.parseInt(args[3]);
