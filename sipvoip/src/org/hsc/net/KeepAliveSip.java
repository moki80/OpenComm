 ...
 * (e.g. toward the seriving proxy/gw or a remote UA). It periodically sends
...
public class KeepAliveSip extends KeepAliveUdp {
	...
	public KeepAliveSip(SipProvider sip_provider, SocketAddress target,
			long delta_time) {
	...
	public void sendToken() throws java.io.IOException { // do send?
		if (!stop && target != null && sip_provider != null) {
			sip_provider.sendMessage(message, sip_provider
					.getDefaultTransport(), target.getAddress().toString(),
					target.getPort(), 127);
		}
	}
	...
	public String toString() {
		...
		if (sip_provider != null) {
			str = "sip:" + sip_provider.getViaAddress() + ":"
					+ sip_provider.getPort() + "-->" + target.toString();
		}
