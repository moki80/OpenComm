...
public class RtpStreamReceiver extends Thread {
	...
	public void run() {
		...
		track.play();
		if (DEBUG)
			println("Playing "+track.getAudioFormat());
		try {
			...
			while (running) {
				try { // read a block of data from the rtp socket
					rtp_socket.receive(rtp_packet);
					 if (DEBUG) System.out.print("CHN");

					 
					// write this block to the output_stream (only if still
					// running..)
//					if (running)
//						output_stream.write(rtp_packet.getPacket(), rtp_packet
//								.getHeaderLength(), rtp_packet
//								.getPayloadLength());
					
					 if (running) { 
						 byte[] pkt=rtp_packet.getPacket(); 
						 int offset=rtp_packet.getHeaderLength(); 
						 int len=rtp_packet.getPayloadLength(); int pos=0; 
						 for (int i=0; i<len; i++) { 
