...
public class RtpStreamTranslator extends Thread {
	 ...
	 *            the thestination port
	 */
	public RtpStreamTranslator(DatagramSocket socket_in,
			DatagramSocket socket_out, String dest_addr, int dest_port) {
		init(socket_in, socket_out, dest_addr, dest_port);
	}

	/** Inits the RtpStreamTranslator */
	private void init(DatagramSocket socket_in, DatagramSocket socket_out,
		...
		try {
			...
			rtp_socket_out = new RtpSocket(socket_out, InetAddress
	...
	public void run() {
		...
		// try {
		// audio_input_stream=javax.sound.sampled.AudioSystem.getAudioInputStream(file);
		...
		try {
			...
			while (running) {
				try { // read a block of data from the rtp_socket_in
					...
					if (running) {
						...
						for (int i = 0; i < len1; i++) {
							...
							// int
							// linear2=G711.ulaw2linear(audio_input_stream.read());
							// linear+=linear2;
							pkt2[pos2++] = (byte) G711.linear2ulaw(linear);
						}
						rtp_packet_out.setPayloadType(rtp_packet_in
								.getPayloadType());
						rtp_packet_out.setSequenceNumber(rtp_packet_in
								.getSequenceNumber());
						rtp_packet_out.setTimestamp(rtp_packet_in
								.getTimestamp());
	...
	public static void main(String[] args) {
		...
		RtpStreamTranslator translator = new RtpStreamTranslator(rport, daddr,
				dport);
		translator.start();
	}
