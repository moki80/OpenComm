 ...
 * output lines (InputStreams). <p/> Each line has an identifier (Object) used
 * as key when adding or removing the line.
 */
public class Mixer {
	/** The splitter lines (as Hashtable of Object->SplitterLine). */
	Hashtable<Object, SplitterLine> splitter_lines;

	/** The mixer lines (as Hashtable of Object->MixerLine). */
	Hashtable<Object, MixerLine> mixer_lines;

	/** Creates a new Mixer. */
	public Mixer() {
		splitter_lines = new Hashtable<Object, SplitterLine>();
		mixer_lines = new Hashtable<Object, MixerLine>();
	}

	/** Close the Mixer. */
	public void close() throws IOException {
		for (Enumeration<SplitterLine> e = splitter_lines.elements(); e.hasMoreElements();) {
			e.nextElement().close();
		}
		for (Enumeration<MixerLine> e = mixer_lines.elements(); e.hasMoreElements();) {
			e.nextElement().close();
		}
		mixer_lines = null;
		splitter_lines = null;
	...
	public OutputStream newInputLine(Object id) throws IOException { // System.err.println("Mixer:
																		// add
																		// input
																		// line:
																		// "+id);
		...
		for (Enumeration e = mixer_lines.keys(); e.hasMoreElements();) {
			...
			if (!mid.equals(id)) { // PipedInputStream is=new
									// PipedInputStream();
	...
	public void removeInputLine(Object id) { // System.err.println("Mixer:
												// remove input line: "+id);
		...
		for (Enumeration e = mixer_lines.keys(); e.hasMoreElements();) {
			Object mid = e.nextElement();
			if (!mid.equals(id)) { // System.err.println("Mixer: SL("+id+"):
									// remove line: "+mid);
				sl.removeLine(mid);
	...
	public InputStream newOutputLine(Object id) throws IOException { // System.err.println("Mixer:
																		// add
																		// output
																		// line:
																		// "+id);
		...
		for (Enumeration e = splitter_lines.keys(); e.hasMoreElements();) {
			...
			if (!sid.equals(id)) { // PipedInputStream is=new
									// PipedInputStream();
				// PipedOutputStream os=new PipedOutputStream(is);
				ExtendedPipedInputStream is = new ExtendedPipedInputStream();
				ExtendedPipedOutputStream os = new ExtendedPipedOutputStream(is);
	...
	public void removeOutputLine(Object id) { // System.err.println("Mixer:
												// remove output line: "+id);
		...
		for (Enumeration e = splitter_lines.keys(); e.hasMoreElements();) {
			Object sid = e.nextElement();
			if (!sid.equals(id)) { // System.err.println("Mixer: ML("+id+"):
									// remove line: "+sid);
				ml.removeLine(sid);
