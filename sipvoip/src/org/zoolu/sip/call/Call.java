...
//		this.log = sip_provider.getLog();
	...
	public void call(String callee) {
		call(callee, null, null, null, null);
	}

	/** Starts a new call, inviting a remote user (<i>callee</i>) */
	public void call(String callee, String sdp, String route) {
		call(callee, null, null, sdp, route);
	}

	/** Starts a new call, inviting a remote user (<i>callee</i>) */
	public void call(String callee, String from, String contact, String sdp, String route) {
		printLog("calling " + callee, LogLevel.HIGH);
		Log.v(Constants.LogConstants.TAG, "Starting Call");
		if (from == null)
			from = from_url;
		if (contact == null)
			contact = contact_url;
		...
		dialog = new InviteDialog(sip_provider, this);
		if (local_sdp != null)
			dialog.invite(callee, from, contact, local_sdp, route);
		else
			dialog.inviteWithoutOffer(callee, from, contact, route);
	}
	...
	/************* ADDED BY RS ***********/
	/** Answers at the 2xx/offer (in the ack message) */
	public void prackWithAnswer(String sdp) {
		local_sdp = sdp;
		dialog.prackWithAnswer(contact_url, sdp);
	}
	/** Answers at the 2xx/offer (in the ack message) */
	public void prackWithoutAnswer() {
//		local_sdp = sdp;
		dialog.prackWithAnswer(contact_url, null);
	}
	/**********************************/
	...
	public void onDlgInviteProvisionalResponse(InviteDialog d, int code,
		...
		if (d != dialog) {
			printLog("NOT the current dialog", LogLevel.HIGH);
			Log.v(Constants.LogConstants.TAG,"Not current dialog");
			return;
		}
		if (sdp != null && sdp.length() != 0)
			remote_sdp = sdp;
		...
		if (code == 183)
			if (listener != null)
				listener.onCallSessInProgress(this, sdp, msg);
	}
