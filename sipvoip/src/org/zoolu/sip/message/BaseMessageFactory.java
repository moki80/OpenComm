...
public abstract class BaseMessageFactory {
	...
	public static Message createRequest(Dialog dialog, String method,
		...
		if (method.equals("PRACK")) {
			String rAck = SipProvider.pickInitialCSeq() + " " + cseq + "INVITE";
			req.setHeader(new Header("RAck", rAck));
			req.removeContacts();
		}
		if (method.equals("ACK")) {
			String rAck = SipProvider.pickInitialCSeq() + " " + cseq;
			req.removeContacts();
		}
		Vector<NameAddress> route = dialog.getRoute();

		if (route != null && route.size() > 0) {
			Log.v(Constants.LogConstants.TAG, "Route values\n"
					+ route.toString());
			Log.v(Constants.LogConstants.TAG, "Route present");
			Vector<String> route_s = new Vector<String>(route.size());
			for (Enumeration<NameAddress> e = route.elements(); e
					.hasMoreElements();) {
	...
	/**
	 * Creates an PRACK request for a 2xx response. Added by Ruchir Singhvi
	 * 
	 * @see #createRequest(Dialog,String,String)
	 */
	public static Message create1xxPrackRequest(Dialog dialog, String body) {
		return createRequest(dialog, SipMethods.PRACK, body);
	}
...
//		if (request_uri == null)
//			request_uri = to.getNameAddress().getAddress();
//		String branch = SipStack.pickBranch();
//		NameAddress contact = null;
//		if (SipStack.contact_url != null)
//			contact = new NameAddress(SipStack.contact_url);
//		return createRequest(SipMethods.ACK, request_uri, to.getNameAddress(),
