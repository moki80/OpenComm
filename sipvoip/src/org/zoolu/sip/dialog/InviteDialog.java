 ...
 * following rules: <br>
 * - both INVITE-offer/2xx-answer and 2xx-offer/ACK-answer modes for incoming
 * calls <br>
...
public class InviteDialog extends Dialog implements TransactionClientListener,
	...
	public InviteDialog(SipProvider sip_provider, Message invite,
		...
		Log.v(Constants.LogConstants.TAG, "In InviteDialog");
	...
	public void invite(String callee, String caller, String contact,
			String session_descriptor, String route) {
		...
		/**************/
		SupportedHeader relHeader = new SupportedHeader("100rel");
		invite.setHeader(relHeader);
		Log.v(Constants.LogConstants.TAG,
				"Invite message after adding 100rel:\n" + invite);
		if (route != null) {
			NameAddress routeAddr = new NameAddress(route);
			RouteHeader routeHeader = new RouteHeader(routeAddr);
			invite.addRouteHeader(routeHeader);
		}
		Log.v(Constants.LogConstants.TAG, "Invite message:\n" + invite);
		/**************/
		// do invite
		invite(invite);
	}
	 ...
	 */
	public void inviteWithoutOffer(String callee, String caller,
			String contact, String route) {
		invite_offer = false;
		// invite(callee, caller, contact, null);
		invite(callee, caller, contact, null, route);
	}
	 ...
	 * <p>
	 * Parameters: <br>
	 * - contact : the contact url OR the contact username; if null, the
	 * previous contact is used <br>
	...
	/***************** ADDED BY RS ************/
	/** Sends the ack when offer/answer is in 2xx/ack */
	public void prackWithAnswer(String contact, String session_descriptor) {
		if (contact != null)
			setLocalContact(new NameAddress(contact));
		Message prack = MessageFactory.create1xxPrackRequest(this,
				session_descriptor);

		Log
				.v(Constants.LogConstants.TAG, "\nPRACK message generated\n"
						+ prack);
		prackWithAnswer(prack);
	}

	public void prackWithAnswer(Message prack) {
//		ack_req = prack;
		invite_offer = true;
		PrackTransactionClient prackTransClient = new PrackTransactionClient(
				sip_provider, prack, this);
		prackTransClient.request();
	}
	...
	public void cancel(Message cancel) {
		...
		if (statusIs(D_INVITING) || statusIs(D_ReINVITING)) { // changeStatus(
			// D_CANCELING);
	...
	/**
	 * Redirects the incoming call , specifing the <i>code</i> and
	 * <i>reason</i>. This method can be called when the InviteDialog is in
	...
	public void onReceivedMessage(SipProvider sip_provider, Message msg) {
		...
		if (msg.isResponse()) {
			...
			if (code >= 100 && code < 200) {
				prackWithAnswer(msg.getContactHeader().toString(), msg
						.getBody());
			}
			if (code >= 200 && code < 300) {
	...
	public void onTransProvisionalResponse(TransactionClient tc, Message msg) {
		...
		if (tc.getTransactionMethod().equals(SipMethods.INVITE)) {
			...
			if (msg.getStatusLine().getCode() >= 100
					&& msg.getStatusLine().getCode() < 200) {
				Log.v(Constants.LogConstants.TAG,
						"onTransProvisionalResponse InviteDialog");
	...
	public void onTransSuccessResponse(TransactionClient tc, Message msg) {
		...
		if (tc.getTransactionMethod().equals(SipMethods.INVITE)) {
			...
			changeStatus(D_CALL);
			update(Dialog.UAC, msg);
			if (invite_offer) { // invite_req=MessageFactory.createRequest(
				// SipMethods.ACK,dialog_state,sdp.toString());
			...
			if (!re_inviting) {
				...
				listener.onDlgReInviteSuccessResponse(this, code, statusline
						.getReason(), msg.getBody(), msg);
		} else if (tc.getTransactionMethod().equals(SipMethods.PRACK)) {
//			if (!verifyStatus(statusIs(D_BYEING)))
//				return;
			StatusLine statusline = msg.getStatusLine();
			int code = statusline.getCode();
			verifyThat(code >= 200 && code < 300, "2xx for prack was expected");
			changeStatus(D_INVITING);
