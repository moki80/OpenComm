 ...
 * These types of identifiers characterize respectively: <br>
...
public class SipProvider implements Configurable, TransportListener,
	...
	public boolean addSipProviderListener(Identifier id,
		 ...
		 * list += e.nextElement() + ", "; printLog(listeners.size() +
		 * " listeners: " + list, LogLevel.LOW); }
		 */
		return ret;
	}
	...
	protected void processReceivedMessage(Message msg) {
		try { // logs
			...
			if (listeners.containsKey(key)) {
				SipProviderListener sip_listener = (SipProviderListener) listeners
						.get(key);
