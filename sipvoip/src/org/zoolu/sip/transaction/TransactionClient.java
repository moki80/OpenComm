...
public class TransactionClient extends Transaction {
	...
	public void onReceivedMessage(SipProvider provider, Message msg) {
		if (msg.isResponse()) {
			Log.v(Constants.LogConstants.TAG, "Recvd Message is:\n" + msg);
			...
			if (code >= 100 && code < 200
				...
				if (statusIs(STATE_TRYING))
					...
					if (transaction_listener != null) {
						transaction_listener.onTransProvisionalResponse(this,
								msg);
					}
				return;
			}
			if (code >= 200 && code < 700
					&& (statusIs(STATE_TRYING) || statusIs(STATE_PROCEEDING))) {
				Log.v(Constants.LogConstants.TAG, "resp is:\n" + code);
				retransmission_to.halt();
				transaction_to.halt();
				// if (statusIs(STATE_PROCEEDING)) {
				Header rSeq = msg.getHeader("RSeq");
				Log.v(Constants.LogConstants.TAG, "RSEQ is: " + rSeq);
				if (rSeq != null) {
					Log.v(Constants.LogConstants.TAG,
							"Setting state to trying due to RSeq");
					changeStatus(STATE_TRYING);
				} else {
					changeStatus(STATE_COMPLETED);
				}
				// }
				Log.v(Constants.LogConstants.TAG, "Status is:\n" + getStatus());
				if (code < 300) {
					if (transaction_listener != null)
