...
/**
 * prack client transaction as defined in RFC 3261 (Section 17.2.1). <BR>
 * An prackTransactionClient is responsable to create a new SIP prack
 * transaction, starting with a prack message sent through the SipProvider and
 * ending with a final response. <BR>
 * The changes of the internal status and the received messages are fired to the
 * TransactionListener passed to the prackTransactionClient object.
 */
public class PrackTransactionClient extends TransactionClient {
	/**
	 * the TransactionClientListener that captures the events fired by the
	 * prackTransactionClient
	 */
	...
	/** end timeout for prack transactions ("Timer D" in RFC 3261) */
	Timer end_to;

	/** Creates a new ClientTransaction */
	public PrackTransactionClient(SipProvider sip_provider, Message req,
	...
	/** Starts the prackTransactionClient and sends the prack request. */
	public void request() {
		...
		Hashtable tmp = sip_provider.getListeners();
		Enumeration tmp1 = tmp.elements();
	...
	public void onReceivedMessage(SipProvider provider, Message msg) {
		if (msg.isResponse()) {
			Log.v(Constants.LogConstants.TAG, "prackTransactionClient");
			int code = msg.getStatusLine().getCode();
			if (code >= 100 && code < 200
					&& (statusIs(STATE_TRYING) || statusIs(STATE_PROCEEDING))) {
				System.out
						.println("onReceivedMessage:prackTransactionClient, 1xx recvd");
			...
			if (code >= 200 && code < 300
				...
				System.out
						.println("transaction id for prack:" + transaction_id);
